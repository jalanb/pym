...
def format_argnames(argnames):
        return ', '.join((((isinstance(name, tuple) and ('(%s)' % format_argnames(name))) or name) for name in argnames))
def format_ass(node):
...
class ASTVisitor(object):
        ...
        def visitWith(self, node):
                raise NotImplementedError('The `with` keyword is not supported.')def visitAugAssign(self, node):
                yield self.visit(node.node)
                yield (' %s ' % node.op)
                yield self.visit(node.expr)
                yield None
