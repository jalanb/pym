	...
	if write_file == 'cpp':
		...
		for header in needed_headers:
			...
			if included_header not in included_headers:
				included_headers += [included_header]
		included_headers += ['# include "logger.h"']
	...
	for t in used_types:
		...
		if not header:
			...
			if not header_path.isfile():
				missing += ['%s -> %s' % (changed, header_path)]
...
def change_cpp(text):
	...
	for line in text.splitlines():
		...
		if vector_set.search(line): line = vector_set.sub(r'\1->at(\2) = \3',line)
		if vector_get.search(line): line = vector_get.sub(r'\1->at(\2)',line)
		result += [line]
	return '\n'.join(result)
