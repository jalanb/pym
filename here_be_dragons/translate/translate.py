...
def write_method_declaration(return_declaration, identifier, formal_args, throws_list):
    ...
    return "%s %s::%s %s" % (return_declaration, class_name, identifier, formal_args)
...
def change_cpp(text):
    ...
    if class_name != "Rias1221Messages":
        text = text.replace(
            ...
            "Rias1221Messages::ATTRIBUTE_VALUE_CHANGE_EVENT",
        )
...
def write_attribute_declaration(i, j):
    ...
    if j:
        ...
        if "static" in attribute_parts:
            attribute_parts = [
                a for a in attribute_parts if ":" not in a and a not in ["static"]
            ]
...
def make_header(java, header, name):
    ...
    with open(header, "w") as stream:
        stream.write("# ifndef %s_H " % name.upper())
        stream.write("# define %s_H " % name.upper())
        stream.write(fred_text)
        stream.write("# endif")
