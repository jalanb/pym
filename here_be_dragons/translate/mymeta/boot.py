...
"""
from mymeta.runtime import OMetaBase, ParseError
import string
from mymeta.builder import AstBuilder


class BootOMetaGrammar(OMetaBase):
    """
    The bootstrap grammar, generated from L{mymeta.grammar.OMetaGrammar} via
    L{mymeta.builder.PythonBuilder}.
    """
    ...
    def rule_application(self):
        ...
        def _G__or_17():
            ...
            _locals["args"]
            return eval(
                "self.builder.apply(name, self.name, *args)", self.globals, _locals
            )
    ...
    def rule_rulePart(self):
        ...
        _locals["requiredName"] = self.apply(
            "anything",
        )
