...
class OMetaBase(object):
    ...
    def apply(self, ruleName, *args):
        ...
        if r is not None:
            try:
                ...
                if "self._apply" in error:
                    message = "\n\t%s%s" % (message, e)
                else:
                    i = self.input.position
                    try:
                        before = "".join(self.input.data[:i])
                    except TypeError:
                        before = str(self.input.data[:i])
                    try:
                        after = "".join(self.input.data[i:])
                    except TypeError:
                        after = str(self.input.data[i:])
                    message = "\n%s|%s\n\t%s\n\t\tParseError(%s)" % (
                        before,
                        after,
                        message,
    ...
    def _apply(self, rule, ruleName, args):
        ...
        if memoRec is None:
            ...
            memoRec = self.input.setMemo(ruleName, [rule(), self.input])
            if lr.detected:
                sentinel = self.input
                while True:
                    try:
                        ...
                        memoRec = oldPosition.setMemo(ruleName, [ans, self.input])
                    except ParseError:
                        break
            self.input = oldPosition
